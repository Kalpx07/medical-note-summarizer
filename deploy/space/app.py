"""
Medical Note Summarizer — Hugging Face Space (CPU) edition.

Runs the LoRA-fine-tuned Llama 2 7B (fused, GGUF Q4_K_M) with llama.cpp on the
free CPU tier. Generation takes 1-2+ minutes per summary — this Space trades
speed for a permanent free demo.
"""
...
FT_REPO = os.getenv("MODEL_REPO", "imkalpx/llama2-med-summarizer-gguf")
FT_FILE = os.getenv("MODEL_FILE", "med-summarizer-Q4_K_M.gguf")
BASE_REPO = "TheBloke/Llama-2-7B-GGUF"
...
def load_model(which: str):
    ...
    repo, fname = (FT_REPO, FT_FILE) if which == "finetuned" else (BASE_REPO, BASE_FILE)
...
def truncate(text: str) -> str:
    for marker in ("</s>", "CLINICAL NOTE:", "You are a clinical"):
        ...
        if idx != -1:
            text = text[:idx]
    # FOLLOW-UP values are plain phrases; anything past other punctuation is junk
    text = re.sub(r"(FOLLOW-UP:\s*[A-Za-z0-9\- ]*).*", r"\1", text, flags=re.S)
    text = re.sub(r"[^\x20-\x7E\n]+", "", text)
    return text.strip()


def summarize(note: str, which: str) -> str:
    llm = load_model(which)
    out = llm(
...
.med-hero {
    ...
    margin-bottom: 1.6rem; box-shadow: 0 8px 24px rgba(2,132,199,.25);
}
...
.med-badge {
    ...
    padding:.25rem .8rem; font-size:.78rem; font-weight:600; color:#fff; margin-right:.5rem;
}
.med-card {
...
.med-chip { font-size:.7rem; font-weight:700; border-radius:999px; padding:.18rem .6rem; text-transform:uppercase; letter-spacing:.05em; }
...
with col_opts:
    compare_base = st.checkbox(
        "Also run base Llama 2 for comparison (roughly doubles the wait)", value=False)

if run and note.strip():
    cols = st.columns(2 if compare_base else 1, gap="large")

    with cols[0]:
        st.markdown('<div class="med-model-head">Fine-tuned model '
                    '<span class="med-chip ft">Ours</span></div>', unsafe_allow_html=True)
        with st.spinner("Generating on free CPU hardware — this takes 1-2 minutes..."):
            try:
                ft = summarize(note, "finetuned")
                render_summary(ft)
    ...
    if compare_base:
        with cols[1]:
            st.markdown('<div class="med-model-head base">Base Llama 2 7B '
                        '<span class="med-chip base">Baseline</span></div>',
                        unsafe_allow_html=True)
            with st.spinner("Generating baseline summary (1-2 minutes)..."):
                try:
                    base = summarize(note, "base")
                    render_summary(base)
